...
Audit the repository against the standing instructions. Create or refine the minimum decision-log/ADR structure, contribution workflow, definition of done, verification report format, and phase-checkpoint policy. Do not add application code or dependencies. Reconcile contradictions in documentation and surface decisions that require the owner.

Acceptance: contributors have one unambiguous reading order; architecture conflicts have a stop/ADR path; completion evidence is standardized; no speculative implementation exists.
