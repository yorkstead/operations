...
Build secure intake and classification for job documents and archives, with revision association, document type, extraction status, human review, and packet assembly for departments. AI output is untrusted: preserve source provenance, confidence, model/version metadata, and approval. Detect revision, quantity, and material inconsistencies without silently changing job truth.

Acceptance: representative synthetic documents progress from upload to reviewed packet; prompt injection/file safety, failure/retry, authorization, cost/usage logging, and human override are covered.
