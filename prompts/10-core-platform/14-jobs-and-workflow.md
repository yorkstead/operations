...
Implement the canonical job/work-order aggregate, releases/revisions, statuses, assignments, dates, priorities, customer links, and configurable but guarded workflow transitions. Preserve a timeline of meaningful changes. Do not implement detailed production operations yet.

Acceptance: job intake through release readiness works; invalid transitions and stale updates fail safely; revisions are explicit; permissions, concurrency, audit history, and tests are complete.
