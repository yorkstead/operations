# Files, Activity, and Notifications
...
Implement private file metadata/storage ports, authorized upload/download, activity history, in-app notification records, and reliable asynchronous delivery boundaries. Add virus-scanning/quarantine hooks without pretending a scanner exists. Use an outbox or documented equivalent for committed side effects.

Acceptance: file URLs expire and cannot cross tenants; upload limits/type validation exist; operational events render a useful activity trail; retries are idempotent; sensitive content is not logged.
