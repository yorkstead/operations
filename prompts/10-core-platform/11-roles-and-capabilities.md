...
Implement the authorization model in `docs/AUTHORIZATION.md`: capabilities, default roles, organization-specific assignments where justified, server-side policy helpers, and an auditable administration flow. Avoid hardcoded role checks in pages.

Acceptance: allow/deny cases are tested at service and request boundaries; privilege changes invalidate or refresh authorization safely; UI explains unavailable actions; default is deny.
