...
export async function GET() {
  ...
  try {
    if (sessionContext.activeOrganization.isDemo) {
      const metrics = purchasingService.getPurchasingMetrics(sessionContext);
      return NextResponse.json({ metrics });
    }
    const metrics = await purchasingRepository.getMetrics(sessionContext);
    return NextResponse.json({ metrics });
  } catch (err: unknown) {
    return apiErrorResponse(err, { action: "api.purchasing.metrics" });
  }
}
