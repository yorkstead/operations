...
export async function POST(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    if (sessionContext.activeOrganization.isDemo) {
      const po = purchasingService.issuePO(sessionContext, id);
      return NextResponse.json({ purchaseOrder: po });
    }
    const po = await purchasingRepository.issuePurchaseOrder(sessionContext, id);
    return NextResponse.json({ purchaseOrder: po });
  } catch (err: unknown) {
    return apiErrorResponse(err, { action: "api.purchasing.orders.[id].issue" });
  }
}
