...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    if (sessionContext.activeOrganization.isDemo) {
      const article = knowledgeService.getArticle(sessionContext, id);
      return NextResponse.json({ article });
    }
    const article = await knowledgeRepository.getArticle(sessionContext, id);
    return NextResponse.json({ article });
  } catch (err: unknown) {
    return apiErrorResponse(err, { action: "api.knowledge.articles.[id]" });
  }
}
