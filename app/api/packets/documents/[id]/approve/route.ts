...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (sessionContext.activeOrganization.isDemo) {
      const document = packetIntelligenceService.approveExtraction(sessionContext, id, overrides);
      return NextResponse.json({ document });
    }

    const document = await packetIntelligenceRepository.approveExtraction(sessionContext, id, overrides);
    return NextResponse.json({ document });
  } catch (err: unknown) {
    return apiErrorResponse(err, { action: "api.packets.documents.[id].approve" });
  }
}
