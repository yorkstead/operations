...
export async function POST(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    if (sessionContext.activeOrganization.isDemo) {
      const quote = quoteService.approveQuote(sessionContext, id);
      return NextResponse.json({ quote });
    }
    const quote = await quotingRepository.approveQuote(sessionContext, id);
    return NextResponse.json({ quote });
  } catch (err: unknown) {
    return apiErrorResponse(err, { action: "api.quoting.quotes.[id].approve" });
  }
}
