...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    if (sessionContext.activeOrganization.isDemo) {
      const pkg = packagingService.getPackage(sessionContext, id);
      return NextResponse.json({ package: pkg });
    }
    const pkg = await packagingRepository.getPackage(sessionContext, id);
    return NextResponse.json({ package: pkg });
  } catch (err: unknown) {
    return apiErrorResponse(err, { action: "api.packaging.containers.[id]" });
  }
}
